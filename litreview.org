...
** RStudio Shiny
   ...
   + URL :: http://rstudio.org/, http://rstudio.github.com/shiny/tutorial/,[[https://github.com/rstudio/shiny][ https://github.com/rstudio/shiny]] [PRIVATE]
   + Author :: Josh Paulson, Joe Cheng, JJ Allaire
   + Status :: Active
   + Category :: Application Server

   To install the software the following code needs to be executed:

   : options(repos=c(RStudio="http://rstudio.org/_packages", getOption("repos")))
   : install.packages("shiny")

   As a result, we observe that the current location of the ~shiny~
   package is at:
   http://rstudio.org/_packages/src/contrib/shiny_0.1.2.tar.gz
...
** d3
   ...
   ~d3.select("#geom_point.points.2")~ and are instead forced to grab
   the node first using ~document.getElementById~.
