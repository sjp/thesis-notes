...
The key advantage of \gridSVG{} is that we can identify its
output. This was previously performed by translating the name of the
\grid{} object to an \code{id} attribute on the generated SVG
element. The fact that SVG is an XML-based image format means that if
we are to identify SVG output by name, we are required to produce SVG
\code{id} attributes that are unique. The previous implementation was
not guaranteed to produce unique \code{id} attributes. This chapter
describes how \gridSVG{} retains the names associated with grobs and
viewports, along with the difficulties in doing so.
...
and the display list that \grid{} records for the image are shown in
\autoref{fig:simpleImage}.

<<simpleImage, fig.keep = "last", fig.show = "hold", echo = -5, fig.cap = "A \\grid{} image with viewports and a single grob.">>=
...
What we can see in \autoref{fig:simpleImage} is that there are three
viewports, named \code{ROOT}, \code{a}, and \code{b}, one of which
shares its name with a circle called \code{a}. The \code{ROOT}
viewport is a viewport that \grid{} creates by default that
corresponds to the entire drawing canvas. This explains why
\code{ROOT} exists in the display list without explicitly being
created.
...
cat('  <g id="a::b.1">')
cat('    <g id="a.2">')
cat('      <circle id="a.2.1" cx="252" cy="252" r="126" fill="rgb(70,13...')
cat('    </g>')
cat('  </g>')
cat('</g>')
@
...
\autoref{chap:coords}), because the coordinate information will be
paired with the SVG \code{id} that was generated by \gridSVG{}. This
is necessary because each time a viewport path is visited there may be
a different coordinate system in use. For example, consider the case
where we create two different viewports that share the same name but
...
\code{id} attributes that \gridSVG{} will generate in this situation
...
coordinate information that has been exported to JSON
\autocite{JSO13}, a structured data format.
...
viewports have been retained. We know this is the case because the
...
object that we are representing. However, in this example, because this
...
\code{grid.circle()}. An example of this is shown in
\autoref{fig:subGrobEx}.

<<subGrobEx, fig.cap = "A single circle grob that draws three circles.">>=
...
identical names. The solution \gridSVG{} applies is to use an integer
...
<<subGrobIdEx, fig.cap = "A single polyline grob drawing 5 separate lines.">>=
...
\autoref{fig:subGrobIdEx} shows that a single call to
\code{grid.polyline()} has produced 5 distinct lines. This is because
of \code{grid.polyline()}'s \code{id} parameter which determines the
sub-grob that each line coordinate belongs to. The SVG output is shown
below, and demonstrates that the same rule applies to grobs with
vectorised parameters and to those with an \code{id} parameter.
...
<<usePathsTestImage, fig.cap = "A simple \\grid{} image featuring a grob tree and a tree of viewports.">>=
...
What has been drawn in \autoref{fig:usePathsTestImage} are two grobs,
a circle and a rectangle. They are the only children in a single tree
of grobs called \code{gt}. This tree has been drawn inside the
viewport path \code{a::b}. Because we have trees of content, we can
easily compare the effect of each option. We will first look at the
output when we only want viewport paths to be used.
...
not use paths for their names so they are exported as \code{c.1} and
...
tmpsub <- getNodeSet(tmp, "//*[@id='a.1']")[[1]]
splitlines <- strsplit(saveXML(tmpsub, file = NULL), "\n")[[1]]
...
is the case is when using \code{id}s within CSS selectors
\autocite{CSSSel11}. This is because the colon character is a special
character in CSS \autocite{CSS11}, as it prefixes a
...
replacing it with \code{\textbackslash:\textbackslash:}. Ideally we
would like to avoid performing any escaping by using a different
separator. This section discusses how custom separators can be used by
\gridSVG{} when it exports an SVG image.
...
\item[\code{id}] The separator between the name given to a grob or
  viewport and an additional integer suffix, used for the purposes of
...
\code{usePaths} being set to \code{both}. The relevant output is shown
below:

<<svgSeparators, fig.keep = "none", echo = 1:2>>=
pushViewport(vpt)
grid.draw(gt)
cat('R> grid.export(usePaths = "both")\n')
tmp <- grid.export("", usePaths = "both")$svg
...
that are the same, creating invalid SVG, for which \gridSVG{} is
providing a warning message.

Careful consideration should be made when using this parameter because
it is the only parameter that has the potential to produce invalid SVG
documents. In fact, the need to change this parameter from the default
of \code{TRUE} is rarely necessary when mapping information is used.

\subsection{Prefixes}

It has been shown earlier in this chapter that \gridSVG{} is now able
to generate documents with unique \code{id} attributes. This fulfils
the requirements for producing valid SVG documents. When each image is
embedded in a web page using the \code{<img>} tag, or viewed
standalone in other software, these \gridSVG{} images work
well. However, when \gridSVG{} images are inserted inline into HTML,
or inserted into other XML or SVG documents, the uniqueness of
\code{id} attributes cannot be guaranteed. This is because the
documents that a \gridSVG{} image is inserted into may already have
the same \code{id} attributes as those present in the \gridSVG{}
image. The problem will be demonstrated with two simple images (see
\autoref{fig:firstGridCollide} and
\autoref{fig:secondGridCollide}). The \grid{} images will be exported
to SVG and inserted into an HTML document inline.

<<firstGridCollide, fig.width = 1, fig.height = 1, out.width = "1in", out.height = "1in", fig.keep = "last", fig.cap = "A \\grid{} image with a circle grob named ``mygrob''.">>=
grid.newpage()
grid.circle(name = "mygrob")
firstImage <- grid.export("")$svg
@
<<secondGridCollide, fig.width = 1, fig.height = 1, out.width = "1in", out.height = "1in", fig.keep = "last", fig.cap = "A \\grid{} image with a rect grob named ``mygrob''.">>=
grid.newpage()
grid.rect(name = "mygrob")
secondImage <- grid.export("")$svg
@

When we combine the content of these two images into a single HTML
document, their \code{id} attributes will no longer be unique. The relevant subset of the document is shown below:

<<htmlInlineCollide, echo = FALSE>>=
cat('R> cat(paste0("<html><body>",')
cat('R+            saveXML(firstImage, file = NULL),')
cat('R+            saveXML(secondImage, file = NULL),')
cat('R+            "</body></html>"))')
cat('<html>')
cat('  <body>')
cat('    <svg ... >')
...
cat('      <g id="gridSVG" ... >')
cat('        <g id="mygrob.1">')
cat('          <circle id="mygrob.1.1" cx="48" cy="48.01" r="48"/>')
cat('        </g>')
cat('      </g>')
cat('      ...')
cat('    </svg>')
...
cat('      <g id="gridSVG" ... >')
cat('        <g id="mygrob.1">')
cat('          <rect id="mygrob.1.1" x="0" y="0"')
cat('                width="96" height="96.02"/>')
cat('        </g>')
cat('      </g>')
cat('      ...')
cat('    </svg>')
...
The HTML document clearly shows that \emph{all} of the \code{id}
attributes collide. In other words there are two instances of
\code{gridSVG}, \code{mygrob.1} and \code{mygrob.1.1} when they are
required to be unique.

\gridSVG{} cannot be expected to know the \code{id} attributes present
outside the SVG images it generates, but what it can do is apply a
prefix to all \code{id} attributes. This prefix provides a way of
avoiding any cross-document \code{id} collisions. For example, if we
applied a prefix of ``first-'' to the first image, then the generated
grob \code{id} would be \code{first-mygrob.1} instead of
\code{mygrob.1}. The \code{grid.export()} function has a \code{prefix}
argument that is set to \code{""} by default, but changing it allows
us to prefix all \code{id} attributes with a character string of our
choice.

To demonstrate the \code{prefix} parameter, we will revisit the
examples shown in \autoref{fig:firstGridCollide} and
\autoref{fig:secondGridCollide} to produce an HTML document with
unique \code{id} attributes. The images themselves will not be shown
but we will add the prefix ``first-'' to the first image and
``second-'' to the second image.

<<imagesWithPrefix, fig.keep = "none">>=
grid.newpage()
grid.circle(name = "mygrob")
firstImage <- grid.export("", prefix = "first-")$svg
grid.newpage()
grid.rect(name = "mygrob")
secondImage <- grid.export("", prefix = "second-")$svg
@

These images will now be inserted inline into an HTML document, but
only a relevant subset of the output will be shown.

<<htmlNoInlineCollide, echo = FALSE>>=
cat('<html>')
cat('  <body>')
cat('    <svg id="first-" ... >')
cat('      ...')
cat('      <g id="first-gridSVG" ... >')
cat('        <g id="first-mygrob.1">')
cat('          <circle id="first-mygrob.1.1"')
cat('                  cx="48" cy="48.01" r="48"/>')
cat('        </g>')
cat('      </g>')
cat('      ...')
cat('    </svg>')
cat('    <svg id="second-" ... >')
cat('      <g id="second-gridSVG" ... >')
cat('        <g id="second-mygrob.1">')
cat('          <rect id="second-mygrob.1.1" x="0" y="0"')
cat('                width="96" height="96.02"/>')
cat('        </g>')
cat('      </g>')
cat('      ...')
...
The output shows us that \code{all} of the \code{id} attributes are
now unique. Furthermore, an \code{id} has been given to the
\code{<svg>} elements that holds the value of the associated
prefix. This makes it easy to find a particular SVG image in a web
page because it is simply the prefix that we used to generate it.

The advantages of using prefixing have been discussed and although
they are particularly useful for including \gridSVG{} images inline in
HTML, there are other applications for prefixes. Another example would
be that we can combine pieces of two \gridSVG{}-generated SVG images
in a single SVG image, while ensuring uniqueness amongst the \code{id}
attributes.

\subsection{Classes}

The majority of this chapter is concerned with the \code{id}
attributes applied to SVG elements. This is useful for identifying
individual components of a \gridSVG{} plot but it does not assist with
identifying \emph{groups} of graphical components unless a reasonable
naming scheme has been applied to the \grid{} scene. SVG allows a
\code{class} attribute to be added to any element that determines
membership within a whitespace-separated list of groups. Fortunately,
all \grid{} grobs and viewports are S3 objects, which means that they
possess a character vector of class names. We can use these class
names to add a \code{class} attribute to the SVG elements that
\gridSVG{} generates. The classes that an S3 object is a member of can
be inspected by calling \code{class()}. We will demonstrate this by
applying \code{class()} to some \grid{} objects.
...
The output from the \code{class()} function shows us that all of the
\grid{} grobs are members of the \code{grob} class. Furthermore, both
$x$ and $y$ axis grobs are members of the \code{axis} class. This
shows us that if we can use these classes in SVG then we have a way of
selecting groups of related SVG content. \gridSVG{} caters for this
with the \code{addClasses} parameter for \code{grid.export()}. When
this parameter is \code{TRUE}, the class of a \grid{} object is
transferred to its associated SVG output. We will demonstrate this
with a simple image, as shown in \autoref{fig:gridPreClass}.

<<gridPreClass, echo = -c(7,8), fig.show = "hold", fig.keep = "last", fig.cap = "A \\grid{} image composed of classed objects.">>=
pushViewport(viewport(width = 0.6, height = 0.6,
                      name = "wrapper"))
grid.rect(name = "border")
grid.circle(r = 0.3, name = "circ")
grid.xaxis(name = "xaxis")
grid.yaxis(name = "yaxis")
popViewport()
cat('R> grid.export("image-with-classes.svg", addClasses = TRUE)')
classedSVG <- grid.export("", addClasses = TRUE)$svg
@

When exporting SVG with \code{addClasses} set to \code{TRUE}, the
classes of all \grid{} objects in the image are retained as class
attributes. A relevant subset of the SVG image is shown below.

<<classOutput, echo = FALSE>>=
cat('<g id="wrapper.1" class="pushedvp viewport">')
cat('  <g id="border.1" class="rect grob gDesc">')
cat('    ...')
cat('  </g>')
cat('  <g id="circ.1" class="circle grob gDesc">')
cat('    ...')
cat('  </g>')
cat('  <g id="xaxis.1" class="forcedgrob xaxis axis gTree grob gDesc">')
cat('    <g id="major.1" class="lines grob gDesc">')
cat('      ...')
cat('    </g>')
cat('    <g id="ticks.1" class="segments grob gDesc">')
cat('      ...')
cat('    </g>')
cat('    <g id="labels.1" class="text grob gDesc">')
cat('      ...')
cat('    </g>')
cat('  </g>')
cat('  <g id="yaxis.1" class="forcedgrob yaxis axis gTree grob gDesc">')
cat('    <g id="major.2" class="lines grob gDesc">')
cat('      ...')
cat('    </g>')
cat('    <g id="ticks.2" class="segments grob gDesc">')
cat('      ...')
cat('    </g>')
cat('    <g id="labels.2" class="text grob gDesc">')
cat('      ...')
cat('    </g>')
cat('  </g>')
cat('</g>')
...
The output clearly shows how the class of a \grid{} grob has been
transferred to an SVG \code{class} attribute. This may be useful for
styling classes of SVG content differently. For example, we could
insert the following SVG and CSS snippet into the document to change
how it appears.

<<cssSnippet, echo = FALSE>>=
cat('<style type="text/css">')
cat('  .axis  { stroke: blue;      }')
cat('  .yaxis { stroke-width: 5px; }')
cat('</style>')
cssNode <- newXMLNode("style", attrs = list(type = "text/css"),
                      newXMLTextNode(".axis { stroke: blue; } .yaxis { stroke-width: 5px; }"))
invisible(addChildren(classedSVG, cssNode, at = 0))
@

This CSS code is designed to make all axes blue, but only the $y$ axes
will be thicker with a stroke width of 5 pixels. This is shown in
\autoref{fig:cssImage}.

\begin{figure}[H]
  \centering
    \includegraphics[width=3in, height=3in]{images/classed.pdf}
  \caption{A \gridSVG{} image with additional styling performed using CSS classes.}
  \label{fig:cssImage}
\end{figure}

It is not only easy to style content with CSS classes, but also to
select and manipulate related SVG nodes in both \R{} and a web
browser. This is because we can use CSS selectors to query and collect
SVG nodes. These were actually already shown in the CSS snippet
described earlier because we were able to make all axes blue by using
the selector \code{.axis}. This same selector can be used to find the
SVG nodes in \proglang{JavaScript} using the \code{querySelector()}
and \code{querySelectorAll()} methods. In \R{}, this can be achieved
using the \pkg{selectr} package. More information about CSS selectors
will be discussed in \autoref{chap:selectr}.

\subsection{Output Annotation}

We have shown how \gridSVG{} can be used to modify its SVG
...
path. We can make more sense of an SVG document generated by
\gridSVG{} if we know the values of separators used when
exporting. This is also particularly useful for debugging a \gridSVG{}
image.
...
metadataText <- strsplit(saveXML(tmp, file = NULL), "\n")[[1]]
cat(paste0(sapply(metadataText, lineTrim, side = "right"), collapse = "\n"))
@

The metadata shows us exactly how the image was drawn. In particular,
the \code{gridsvg:argument} elements tell us how the \code{id}
attributes were controlled. We can see that as this particular image
was exported, it was ensuring that unique names were being used, and
the only paths it was generating were for viewport
paths. Additionally, we can also see the values of the separators when
the image was being exported.
...
in \proglang{JavaScript}. In \R{}, we might want to perform some
post-processing on the XML nodes that a grob maps to. If the \code{id}
can be retrieved easily then performing this task is far simpler than
writing an XPath expression \autocite{XPL99}. Similarly, if we want to
perform some modification on an SVG image in the browser, using tools
like \pkg{D3}, then knowing what content we are trying to select is
...
<<mappingSimpleImage, echo = 1:4, fig.keep = "last", fig.show = "hold", warning = FALSE, fig.cap = "A simple \\grid{} image that has been exported to SVG.">>=
grid.newpage()
pushViewport(viewport(name = "a"))
grid.rect(name = "b")
grid.circle(name = "b")
...
\proglang{JavaScript} library like \pkg{D3} or \pkg{jQuery}
\autocite{jQu13}, we can select the content immediately by just using
the exported CSS selector.
...
this is done we can apply the mapping information by using
\code{getSVGMappings()}.
...
In this example, it is shown that the function always returns an array
...
modify content, perhaps using a transition. All that is required is
...
In \R{}, the use of the \pkg{XML} package \autocite{Lan13} is more
familiar, so we can use XPath expressions instead of CSS selectors.
...
With the development of retaining name mapping information, it is
easier to manipulate SVG images that have been exported by \gridSVG{}.
