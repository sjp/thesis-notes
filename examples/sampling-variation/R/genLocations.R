...
genSampleLocs <- function(samps) {
    ...
    pdf(file = NULL)
    grid.newpage()
    tmpvp <- viewportCreate("wrapper::animation.field::sample.3")
    tmpvp$name <- "sample"
    pushViewport(tmpvp)
...
genStatPoints <- function(stats) {
    ...
    pdf(file = NULL)
    grid.newpage()
    tmpvp <- viewportCreate("wrapper::animation.field::stat.2")
    # Need to change names, otherwise it is assumed to be a path...
    tmpvp$name <- "stat"
    pushViewport(tmpvp)
    ...
    pts <- getNodeSet(svgdoc, "//svg:g[@id = 'stat-points']",
...
genSampleStatData <- function(stats) {
    ...
    svgdoc <- gridToSVG(NULL, "none", "none", res = 96)$svg
    ghostsTpl <- getNodeSet(svgdoc, "//svg:g[@id = 'samplePlot-ghosts']",
                            c(svg = "http://www.w3.org/2000/svg"))[[1]]
    lineTpl <- getNodeSet(svgdoc, "//svg:g[@id = 'samplePlot-lines']",
                          c(svg = "http://www.w3.org/2000/svg"))[[1]] 
    dev.off()

    svg.header <- '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="3" height="3">'
    svg.footer <- '</svg>'
